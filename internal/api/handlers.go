...
func (s *Server) handleLease(w http.ResponseWriter, r *http.Request, id, op string) {
	...
	switch op {
	case "acquire":
		status, body, err = s.svc.AcquireLease(r.Context(), opID(r), id, lreq)
	case "move":
		status, body, err = s.svc.MoveLease(r.Context(), opID(r), id, lreq)
	case "renew":
		status, body, err = s.svc.RenewLease(r.Context(), opID(r), id, lreq)
	default:
		writeError(w, &domain.DomainError{Code: domain.ErrNotFound, State: domain.StatePendingLock})
		return
	}
